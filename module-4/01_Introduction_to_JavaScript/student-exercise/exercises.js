...
function lastDigit(x, y) {
  ...
  let xSecond = x.toString();
  let ySecond = y.toString();
  if (xSecond.charAt(xSecond.length-1) == ySecond.charAt(ySecond.length-1)) {
    return result;
  }
  else {
    return falseResult;
...
6. **frontAgain** Given a string, return true if the first 2 chars in the string also appear
    at the end of the string, such as with "edited".
		...
		frontAgain("ed") → true
*/
function frontAgain(fa){
    let length = 2;
    let positionBeg = fa.slice(0,2);
    let positionEnd = fa.slice(fa.length -2,fa.length);
