...
-- ROUNDING
-- **********************
select round(gnp/population, 2) FROM country WHERE gnp > 0;
...
SELECT district, SUM(population), AVG(population), MIN(population), MAX(population) COUNT(populatuon) FROM city
WHERE countrycode = 'USA'
GROUP BY district
ORDER BY district;
